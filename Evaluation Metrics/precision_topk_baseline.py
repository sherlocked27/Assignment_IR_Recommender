				...
				for k in range(0,100):
					if k==i:
						...
						if i<k and sim[i][k-i]>0 and data[k][top_50_index[j]]!=99:
							sum1 = sum1 + (float(data[k][top_50_index[j]] - baseline[k][top_50_index[j]])*float(sim[i][k-i]))
							sum2 = sum2 + float(sim[i][k-i])
						elif i>k and sim[i-k][k]>0 and data[k][top_50_index[j]]!=99:
							sum1 = sum1 + (float(data[k][top_50_index[j]]-baseline[k][top_50_index[j]])*float(sim[i-k][k]))
...
if __name__=='__main__':
	...
	pt.topk_baseline()
"""
Precision Top 50 (Movie ratings of each user)  0.2033023392686095
"""